...
using Distributions, Turing, MCMCChains, Plots, StatsPlots, LinearAlgebra
...
```{julia}
c = c |> @mutate(treatment = 1 + _.prosoc_left + 2 * _.condition) |> DataFrame
```
...
```{julia}
alphas = DataFrame(MCMCChains.group(chains, :alpha)) |> @select(-:chain, -:iteration) |> DataFrame
alphas = stack(alphas)
```

```{julia}
alpha_preds = alphas |> @mutate(prob = logistic(_.value))
```
...
```{julia}
betas = DataFrame(MCMCChains.group(chains, :beta)) |> @select(-:chain, -:iteration) |> DataFrame
